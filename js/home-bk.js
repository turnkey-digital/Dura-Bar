...
function inUSA(lat1, lon1) {
    ...
    for (i = 0; i < Object.keys(distances).length; i++) {
        (currentLocation.distance > distances[i].distance ? updatecurrentLocation(distances[i]) : null)

        function updateCurrentLocation(update) {
            currentLocation = {
                "name": update.name,
...
function updateSection(location) {
